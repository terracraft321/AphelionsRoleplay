...
bool GiveSpawnResources( CRules@ this, CBlob@ blob, CPlayer@ player, CTFPlayerInfo@ info )
{
	...
	if (isClassTypeBuilder(blob))
	{
		ret = SetMaterials( blob, "mat_wood", 100 ) || ret;
		ret = SetMaterials( blob, "mat_stone", 50 ) || ret;
	...
	else if (isClassTypeMarksman(blob))
	{			
	...
	else if (isClassTypeMage(blob))
	{
		ret = SetMaterials( blob, "mat_energyrunes", 10 ) || ret;
...
void onRender(CRules@ this)
{
    ...
    if (b !is null && this.exists(propname) )
    {
        ...
        else if (next_items > getGameTime())
        {
            ...
            GUI::DrawText( "Next Resupply in " + secs + " " + units + "!",
