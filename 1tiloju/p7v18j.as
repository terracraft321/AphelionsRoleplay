...
void onTick( CBrain@ this )
{
	...
	if (delay == 0)
	{
        ...
        if (target !is null)
        {
            ...
            // chase target
            if (getDistanceBetween(target.getPosition(), blob.getPosition()) > blob.getRadius() + blob.get_f32( "attack distance" ))
            {
...
CBlob@ GetBestTarget( CBrain@ this, CBlob@ blob, f32 radius )
{
	...
	for(int step = 0; step < nearBlobs.length; ++step)
	{
		...
		if    (candidate is null || candidate.getName() == "dungeon_entrance" || candidate.getName() == "dungeon_exit") break;

	    f32 priority = getTargetPriority(blob, candidate);
		if (priority > highest_priority && !candidate.hasTag("dead"))
		{
			if (isTarget(blob, candidate) && !isTeamFriendly(blob.getTeamNum(), candidate.getTeamNum()))
			{
...
void DestroyAttackableObstructions( CBrain@ this, CBlob@ blob )
{
	...
	if (getMap().rayCastSolid(blob.getPosition(), blob.getAimPos(), col))
	{
		...
		if (isTarget(blob, obstruction) && this.getBlob().getTeamNum() != blob.getTeamNum())
		{
