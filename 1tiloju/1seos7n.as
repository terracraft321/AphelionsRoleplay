...
void LoadSprites( CSprite@ this )
{
    string texname = this.getBlob().getSexNum() == 0 ?
                    "../Mods/" + RP_NAME + "/Entities/Characters/Classes/Marksman/Handcannoneer/Sprites/HumanHandcannoneerMale.png" :
                    "../Mods/" + RP_NAME + "/Entities/Characters/Classes/Marksman/Handcannoneer/Sprites/HumanHandcannoneerFemale.png";
	
	const string armour = getEquipmentSlotItem(this.getBlob(), SLOT_ARMOUR);
	
	if(this.getBlob().getTeamNum() == RACE_DWARVES)
	{
		texname = this.getBlob().getSexNum() == 0 ?
				 "../Mods/" + RP_NAME + "/Entities/Characters/Classes/Marksman/Handcannoneer/Sprites/DwarfHandcannoneerMale.png" :
				 "../Mods/" + RP_NAME + "/Entities/Characters/Classes/Marksman/Handcannoneer/Sprites/DwarfHandcannoneerFemale.png";
	}
	else if(this.getBlob().getTeamNum() == RACE_ORCS)
	{
		texname = this.getBlob().getSexNum() == 0 ?
				 "../Mods/" + RP_NAME + "/Entities/Characters/Classes/Marksman/Handcannoneer/Sprites/OrcHandcannoneerMale.png" :
				 "../Mods/" + RP_NAME + "/Entities/Characters/Classes/Marksman/Handcannoneer/Sprites/OrcHandcannoneerFemale.png";
	}
	else if(this.getBlob().getTeamNum() == RACE_UNDEAD)
	{
		texname = "../Mods/" + RP_NAME + "/Entities/Characters/Classes/Marksman/Handcannoneer/Sprites/UndeadHandcannoneer.png";
	}
...
void onTick( CSprite@ this )
{
	...
	if (!blob.get( "handcannoneerInfo", @handcannoneer )) {
		return;
	}
	
	doRopeUpdate(this, blob, handcannoneer);
    ...
    if (firing)
    {
        ...
        if (!handcannoneer.has_ammunition || handcannoneer.charge_state == HandcannoneerParams::no_ammunition)
        {
            string animname = "no_arrow";

            if (handcannoneer.charge_time == HandcannoneerParams::ready_time)
            {
                animname = "fired";
            }

            u16 frontframe = 0;
            f32 temp = Maths::Min( handcannoneer.charge_time, HandcannoneerParams::ready_time );
            f32 ready_tween = temp / HandcannoneerParams::ready_time;
		...
		else if (handcannoneer.charge_state == HandcannoneerParams::readying)
		{
			u16 frontframe = 0;
			f32 temp = handcannoneer.charge_time;
			f32 ready_tween = temp / HandcannoneerParams::ready_time;
			armangle = armangle * ready_tween;
			armOffset = Vec2f(-1.0f,4.0f+config_offset+2.0f*(1.0f-ready_tween));
        ...
        else if (handcannoneer.charge_state != HandcannoneerParams::fired)
        {
            u16 frontframe = Maths::Min((handcannoneer.charge_time/(HandcannoneerParams::shoot_period_1+1)),2);
            setArmValues(frontarm,true,armangle,0.1f,"charge",Vec2f(-4.0f*sign,0.0f), armOffset);
            frontarm.animation.frame = frontframe;
            					 
			const f32 arrowangle = (handcannoneer.charge_time > HandcannoneerParams::shoot_period_2) ? armangle + -2.5f + float(XORRandom(500))/100.0f : armangle; // shiver arrow when fully charged
...
void doRopeUpdate(CSprite@ this, CBlob@ blob, HandcannoneerInfo@ handcannoneer)
{
	CSpriteLayer@ rope = this.getSpriteLayer("rope");
	CSpriteLayer@ hook = this.getSpriteLayer("hook");
	
	bool visible = handcannoneer !is null && handcannoneer.grappling;
	...
	Vec2f off = handcannoneer.grapple_pos - blob.getPosition();
	...
	hook.ResetTransform();
	if(handcannoneer.grapple_id == 0xffff) //still in air
	{
		handcannoneer.cache_angle = -handcannoneer.grapple_vel.Angle();
	}
	hook.RotateBy( handcannoneer.cache_angle , Vec2f());
	
	hook.TranslateBy( off );
	hook.SetFacingLeft(false);
	
	//GUI::DrawLine(blob.getPosition(), handcannoneer.grapple_pos, SColor(255,255,255,255));
}
...
void onGib(CSprite@ this)
{
    ...
    CParticle@ Body     = makeGibParticle( "Entities/Characters/Classes/Marksman/Handcannoneer/HandcannoneerGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ), 0, 0, Vec2f (16,16), 2.0f, 20, "/BodyGibFall", team );
    CParticle@ Arm      = makeGibParticle( "Entities/Characters/Classes/Marksman/Handcannoneer/HandcannoneerGibs.png", pos, vel + getRandomVelocity( 90, hp - 0.2 , 80 ), 1, 0, Vec2f (16,16), 2.0f, 20, "/BodyGibFall", team );
    CParticle@ Shield   = makeGibParticle( "Entities/Characters/Classes/Marksman/Handcannoneer/HandcannoneerGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ), 2, 0, Vec2f (16,16), 2.0f, 0, "Sounds/material_drop.ogg", team );
    CParticle@ Sword    = makeGibParticle( "Entities/Characters/Classes/Marksman/Handcannoneer/HandcannoneerGibs.png", pos, vel + getRandomVelocity( 90, hp + 1 , 80 ), 3, 0, Vec2f (16,16), 2.0f, 0, "Sounds/material_drop.ogg", team );
}
