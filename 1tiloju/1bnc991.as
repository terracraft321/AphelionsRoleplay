	...
	if (placedOnWood)
	{
		...
		if(state == hidden)
		{
			...
			if (map.getBlobsInRadius( pos, this.getRadius()*1.0f, @blobsInRadius ))
			{
				for (uint i = 0; i < blobsInRadius.length; i++)
				{
					...
					if (!isTeamFriendly(team, b.getTeamNum()) && canStab(b))
					{	
						state = stabbing;
						timer = delay_stab;
		...
		else if(state == stabbing)
		{
			...
			else
			{
				...
				this.getSprite().PlaySound( "/WoodenSpikesOut.ogg" );
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point )
{
    ...
    if (angle > -180.0f && angle < 0.0f)
    {
        ...
        if (normal.x > 0.5f && verDist < 6.1f && vel.x > 0.5f) {
            this.server_Hit( blob, point, vel*-1, 0.5f, Hitters::spikes, true);
        }
    }
    ...
    else
    {
        ...
        if (normal.y > 0.5f && horizDist < 6.1f && vel.y > 0.5f) {
            ...
            this.server_Hit( blob, point, vel*-1, this.getVelocity().y, Hitters::spikes, true);
        }
    }
}
...
void onHitBlob( CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData )
{
    ...
    {
		CSprite@ sprite = this.getSprite();
        sprite.PlaySound( "/WoodenSpikesCut.ogg" );
