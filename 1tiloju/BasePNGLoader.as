 ...
 */
// ore deposits
const SColor color_deposit_coal_s(255, 220, 137, 97);
const SColor color_deposit_coal_m(255, 196, 114, 78);
const SColor color_deposit_coal_l(255, 152, 72, 33);
const SColor color_deposit_iron_s(255, 151, 115, 97);
const SColor color_deposit_iron_m(255, 123, 95, 81);
const SColor color_deposit_iron_l(255, 102, 80, 69);
const SColor color_deposit_mythril_s(255, 112, 133, 201);
const SColor color_deposit_mythril_m(255, 97, 116, 179);
const SColor color_deposit_mythril_l(255, 77, 98, 166);
const SColor color_deposit_adamantite_l(255, 66, 181, 139);
const SColor color_deposit_random(255, 200, 137, 200); // random ore deposit and size

// chests
const SColor color_chest_iron(255, 142, 128, 134);
const SColor color_chest_steel(255, 184, 169, 176);
const SColor color_chest_gilded(255, 200, 137, 37);
const SColor color_chest_mythril(255, 200, 137, 166);
const SColor color_chest_adamant(255, 80, 208, 155);
const SColor color_chest_random(255, 137, 255, 137); // random chest

// flora
const SColor color_tree_redwood(255, 53, 103, 34);
const SColor color_cactus(255, 147, 127, 14);

// undead
const SColor color_zombie_portal(255, 255, 128, 128);
const SColor color_skeleton(255, 128, 128, 128);
const SColor color_zombie(255, 159, 151, 151);
const SColor color_zombie_knight(255, 128, 101, 101);

// dungeon related
const SColor color_dungeon_entrance(255, 79, 37, 88);
const SColor color_dungeon_exit(255, 103, 63, 112);
const SColor color_dungeon_chest(255, 196, 17, 18); // dungeon chest
const SColor color_dungeon_chest_noom(255, 150, 149, 149); // noom's chest
const SColor color_dungeon_door(255, 215, 215, 215);
const SColor color_necromancer_minion(255, 44, 14, 58); // minion spawner
const SColor color_mini_portal(255, 128, 64, 64); // portal to noom via undead base
const SColor color_undead_main_spawn(255, 115, 115, 115);
...
const SColor color_green_main_spawn(255, 0, 255, 157);
const SColor color_purple_main_spawn(255, 255, 0, 157); 
const SColor color_orange_main_spawn(255, 255, 205, 97);
...
class PNGLoader
{
	...
	{
		@map = _map;
		@map_random = Random(map.getMapSeed());
	...
	if (pixel == color_tile_ground) {
		...
		else if (pixel == color_undead_main_spawn)
		{
			AddMarker( map, offset, "undead main spawn" );
		}
		...
		{
			if (XORRandom(256) < 128)
			{
			    spawnBlob(map, "flowers", offset, -1);
			}
			else if (Time_Month() == 12)
			{
			    server_CreateBlob("present", -1, map.getTileWorldPosition(offset) + Vec2f(4.0f, 4.0f));
			}
		...
		else if (pixel == color_deposit_coal_s)
		{
			spawnBlob( map, "coal_deposit_s", offset, -1, true);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_deposit_coal_m)
		{
			spawnBlob( map, "coal_deposit_m", offset, -1, true);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_deposit_coal_l)
		{
			spawnBlob( map, "coal_deposit_l", offset, -1, true);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_deposit_iron_s)
		{
			spawnBlob( map, "iron_deposit_s", offset, -1, true);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_deposit_iron_m)
		{
			spawnBlob( map, "iron_deposit_m", offset, -1, true);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_deposit_iron_l)
		{
			spawnBlob( map, "iron_deposit_l", offset, -1, true);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_deposit_mythril_s)
		{
			spawnBlob( map, "mythril_deposit_s", offset, -1, true);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_deposit_mythril_m)
		{
			spawnBlob( map, "mythril_deposit_m", offset, -1, true);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_deposit_mythril_l)
		{
			spawnBlob( map, "mythril_deposit_l", offset, -1, true);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_deposit_adamantite_l)
		{
			spawnBlob( map, "adamantite_deposit_l", offset, -1, true);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_deposit_random)
		{
		    int          oreRand = map_random.NextRanged(100);
			string ore = oreRand < 10 ?  "adamantite" : 
			             oreRand < 30 ?  "mythril" : 
						 oreRand < 60 ?  "coal" : 
			                             "iron";
			
			if (map_random.NextRanged(100) < 50)
			{
				int           sizeRand = map_random.NextRanged(3);
		    	string size = sizeRand == 0 ? "s" : sizeRand == 1 ? "m" : "l";
				
				spawnBlob( map, ore + "_deposit_" + size, offset, -1, true);
			}
			offsets[autotile_offset].push_back( offset );
		}
		
		else if (pixel == color_chest_iron)
		{
			spawnBlob( map, "chest_iron", offset, 5 );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_chest_steel)
		{
			spawnBlob( map, "chest_steel", offset, 5 );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_chest_gilded)
		{
			spawnBlob( map, "chest_gilded", offset, 5 );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_chest_mythril)
		{
			spawnBlob( map, "chest_mythril", offset, 5 );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_chest_adamant)
		{
			spawnBlob( map, "chest_adamant", offset, 5 );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_chest_random)
		{
		    int            chestRand = map_random.NextRanged(100);
			string chest = chestRand < 10 ?  "adamant" : 
			               chestRand < 30 ?  "mythril" : 
						   chestRand < 45 ?  "steel" : 
						   chestRand < 60 ?  "gilded" : 
			                                 "iron";
			
			if (map_random.NextRanged(100) < 33)
			{
				spawnBlob(map, "chest_" + chest, offset, 5);
			}
			offsets[autotile_offset].push_back( offset );
		}
		
		else if (pixel == color_tree_redwood)
		{
			offsets[tree_redwood_offset].push_back( offset );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_cactus)
		{
			spawnBlob( map, "cactus", offset, -1, true);
			offsets[autotile_offset].push_back( offset );
		}
		...
		{
			map.SetTile(offset, CMap::tile_castle_back_moss );
			spawnBlob( map, "zombie_portal", offset, 5);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_skeleton)
		{
			map.SetTile(offset, CMap::tile_castle_back );
			spawnBlob( map, "skeleton", offset, 5);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_zombie)
		{
			map.SetTile(offset, CMap::tile_castle_back );
			spawnBlob( map, "zombie", offset, 5);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_zombie_knight)
		{
			map.SetTile(offset, CMap::tile_castle_back );
			spawnBlob( map, "zombie_knight", offset, 5);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_dungeon_entrance)
		{
			spawnBlob( map, "dungeon_entrance", offset, -1);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_dungeon_exit)
		{
			spawnBlob( map, "dungeon_exit", offset, -1);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_dungeon_door)
		{
			CBlob@ door = spawnBlob( map, "dungeon_door", offset, 5, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_dungeon_chest)
		{
			spawnBlob(map, "chest_dungeon", offset, -1);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_dungeon_chest_noom)
		{
			spawnBlob(map, "chest_noom", offset, -1);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_necromancer_minion)
		{
			AddMarker( map, offset, "necromancer minion spawner" );
		}
		else if (pixel == color_mini_portal)
		{
			map.SetTile(offset, CMap::tile_castle_back_moss );
			spawnBlob( map, "mini_portal", offset, 5);
			offsets[autotile_offset].push_back( offset );
		}
	...
	void handleOffset(int type, int offset, int position, int count)
	{
		...
		else if (type == tree_offset)
		{
			...
			{
				CBlob@ tree = server_CreateBlobNoInit( map_random.NextRanged(30) < 15 ? "tree_pine" : "tree_bushy" );
		...
		else if (type == tree_redwood_offset)
		{
			if (map.isTileSolid( map.getTile(offset + map.tilemapwidth) )) // load trees only at the ground
			{
				CBlob@ tree = server_CreateBlobNoInit( "tree_redwood" );
	...
	void SetupBackgrounds()
	{
		...
		map.AddBackground( "Sprites/Back/BackgroundCastle.png", Vec2f(0.0f, 70.0f), Vec2f(0.6f, 0.6f), color_white );
