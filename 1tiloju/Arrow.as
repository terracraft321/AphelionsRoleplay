...
//maximum is 15 as of 22/11/12 (see 1bjdrm3.as)
...
void onInit( CBlob@ this )
{
    ...
    {
		Animation@ anim = sprite.addAnimation("iron arrow",0,false);
		anim.AddFrame(16 + XORRandom(4));
		if(arrowType == ArrowType::normal_iron)
			sprite.SetAnimation(anim);
	}
	
	{
		Animation@ anim = sprite.addAnimation("steel arrow",0,false);
		anim.AddFrame(20 + XORRandom(4));
		if(arrowType == ArrowType::normal_steel)
			sprite.SetAnimation(anim);
	}
	
	{
		Animation@ anim = sprite.addAnimation("mythril arrow",0,false);
		anim.AddFrame(24 + XORRandom(4));
		if(arrowType == ArrowType::normal_piercing)
			sprite.SetAnimation(anim);
	}
...
void onTick( CBlob@ this )
{
	...
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		{
			if(this.getTickSinceCreated() > 40)
			{
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1 )
{
    ...
    {
		if (!solid && !blob.hasTag("flesh") && (blob.getName() != "mounted_bow" || !isTeamFriendly(this.getTeamNum(), blob.getTeamNum())))
		{
			return;
		}
		...
		f32 dmg = isTeamFriendly(this.getTeamNum(), blob.getTeamNum()) ? 0.0f : getArrowDamage( this, vellen );
		...
		{
			u8  hitter = Hitters::arrow;
			if (arrowType == ArrowType::normal_piercing && XORRandom(512) < 128)
			    hitter = Hitters::piercing_arrow;
			
			dmg = ArrowHitBlob(this, point1, initVelocity, dmg, blob, hitter, arrowType);
			if (dmg > 0.0f)
			{
			    if (arrowType == ArrowType::normal)
				    dmg *= 1.0f;
				else if (arrowType == ArrowType::normal_iron)
				    dmg *= 1.25f;
				else if (arrowType == ArrowType::normal_steel)
				    dmg *= 1.5f;
				else if (arrowType == ArrowType::normal_piercing)
				    dmg *= 1.5f;
				
				if (arrowType == ArrowType::fire)
					this.server_Hit( blob, point1, initVelocity, dmg, Hitters::fire);
				else
					this.server_Hit( blob, point1, initVelocity, dmg, hitter);
			}
		}
...
f32 ArrowHitBlob( CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData, const u8 arrowType )
{
    if (hitBlob !is null)
    {
		...
		if (!hitShield)
		{
			...
			if (!isArrowTypeNormal(arrowType))
				damage = 0.0f;
		}
...
{
	const u8 arrowType = this.get_u8("arrow type");
	bool piercing = arrowType == Hitters::piercing_arrow;
	
	// unbomb, stick to blob
	if (this !is hitBlob && (customData == Hitters::arrow || piercing))
	{
		...
		{	
			...
			int max_ticks_to_stun = isCrossbowBolt(this) ? 12 : 4;
			
			if (piercing)
			    force *= 1.1f;
			if (isCrossbowBolt(this))
				force *= 1.3f;
			
			hitBlob.AddForce( velocity * force );
			
			// stun if shot real close
			if (this.getTickSinceCreated() <= max_ticks_to_stun && speed > ArcherParams::shoot_max_vel * 0.845f && hitBlob.hasTag("player"))
			{
				SetKnocked(hitBlob, isCrossbowBolt(this) ? 30 : 20);
...
bool isCrossbowBolt( CBlob@ this )
{
    return this.hasTag("crossbow bolt");
}
