...
void ReloadSprite( CSprite@ this )
{
    ...
    string texname = "Entities/Industry/Drill/Drill.png";
	if(isTechResearched("Machinery III", blob.getTeamNum()))
	    texname = "Entities/Industry/Drill/DrillAdamant.png";
	else if(isTechResearched("Machinery II", blob.getTeamNum()))
	    texname = "Entities/Industry/Drill/DrillMythril.png";
	else if(isTechResearched("Machinery I", blob.getTeamNum()))
	    texname = "Entities/Industry/Drill/DrillSteel.png";
					...
					if (map.getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 30, distance, this, false, @hitInfos))
					{
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (hi.blob !is null) // blob
							{
								...
								if (int(heat) > getMaxHeat(this) * 0.5f)
								{
							...
							else // map
							{
								if (map.getSectorAtPosition(hi.hitpos, "no build") is null) //dont break no build zone stuff
								{
									...
									if (map.isTileStone(tile ) || map.isTileGold(tile))
									    hit_material = true;
									else if (map.isTileCastle(tile) || map.isTileWood(tile))
...
{
	return heat_max * getDrillMaximumHeatModifier(this.getTeamNum());
}
