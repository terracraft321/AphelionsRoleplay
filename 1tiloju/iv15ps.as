...
void ManageGrapple( CBlob@ this, ArcherInfo@ archer )
{
	...
	if (archer.grappling)
	{
		...
		{
		    const f32 archer_grapple_length_ = archer_grapple_length * getMarksmanGrappleLengthModifier(this.getTeamNum());
			const f32 archer_grapple_range = archer_grapple_length_ * archer.grapple_ratio;
			...
			else if(archer.grapple_id == 0xffff) //not stuck
			{
				...
				while(delta > 0 && !found) //fake raycast
				{
					...
					delta -= step;
					found = checkGrappleStep(this, archer, map, dist, archer_grapple_length_);
				}
...
void ManageBow( CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars )
{
	...
	s32 shoot_period = ArcherParams::shoot_period * getMarksmanFireTimeModifier(this.getTeamNum());
	...
	if (charge_state == ArcherParams::legolas_ready) // fast arrows
	{
		...
		{
			ClientFire( this, charge_time, hasarrow, archer.arrow_type, true, shoot_period_1, shoot_period_2 );
			charge_state = ArcherParams::legolas_charging;
			charge_time = shoot_period - ArcherParams::legolas_charge_time;
	...
	else
	{
		if (charge_state > ArcherParams::readying)
		{
			...
			{
				ClientFire( this, charge_time, hasarrow, archer.arrow_type, false, shoot_period_1, shoot_period_2 );
    ...
    if ( ismyplayer )
    {
		...
		{
			int frame = 0;
		//	print("archer.charge_time " + archer.charge_time + " / " + shoot_period );
			if (archer.charge_state == ArcherParams::readying) {
				frame = 1 + float(archer.charge_time)/float(shoot_period + ArcherParams::ready_time) * 7;
			}
			else if (archer.charge_state == ArcherParams::charging) 
				{
					if (archer.charge_time <= shoot_period) { 
						frame = Maths::Min(int(float(ArcherParams::ready_time + archer.charge_time)/float(shoot_period) * 7), 9);
					}
					else
						frame = 9;
				}
...
bool checkGrappleStep(CBlob@ this, ArcherInfo@ archer, CMap@ map, const f32 dist, const f32 len)
{
	...
	else if( grappleHitMap(archer, map, dist) )
	{
		...
		archer.grapple_ratio = Maths::Max(0.2, Maths::Min( archer.grapple_ratio, dist / len) );
	...
	else
	{
		...
		if (b !is null)
		{
			...
			else if(b.isCollidable() && b.getShape().isStatic())
			{
				...
				archer.grapple_ratio = Maths::Max(0.2, Maths::Min( archer.grapple_ratio, b.getDistanceTo(this) / len ) );
...
void ClientFire( CBlob@ this, const s8 charge_time, const bool hasarrow, const u8 arrow_type, const bool legolas, const s32 shoot_period_1, const s32 shoot_period_2 )
{
	...
	{
		f32 arrowspeed;
		if (charge_time < ArcherParams::ready_time/2+shoot_period_1)
			arrowspeed = ArcherParams::shoot_max_vel * (1.0f/3.0f);
		else if (charge_time < ArcherParams::ready_time/2+shoot_period_2)
