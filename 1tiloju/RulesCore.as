...
shared class RulesCore
{
    ...
    void ChangePlayerTeam(CPlayer@ player, int newTeamNum)
    {
		...
		if (getSecurity().checkAccess_Feature(player, "deny_join")) // feature doesn't work by default so this makes up for it
		{
			KickPlayer(player);
			return;
		}
		
	    if(newTeamNum == 4 && !getSecurity().checkAccess_Feature(player, "donator"))
		   newTeamNum = XORRandom(4);
	    else if(newTeamNum == 5 && !isSponsor(player))
		   newTeamNum = XORRandom(4);
	    
		BaseTeamInfo@ b = getTeam(newTeamNum);
		PlayerInfo@ p = getInfoFromName( player.getUsername() );
