...
void onTick( CBlob@ this )
{
	...
	KnightEncumbrance(this, moveVars);
			...
			( pressed_a2 || (specialShieldState) ) )
    {
		...
		u32 glide_down_time = KnightVars::glide_down_time * getKnightGlideTimeModifier(this.getTeamNum());
        ...
        else
        {
            bool forcedrop = (vel.y > Maths::Max(Maths::Abs(vel.x), 2.0f) &&
                              moveVars.fallCount > glide_down_time);
            ...
            if (knight.state == KnightStates::shieldgliding && !this.isInWater() && !forcedrop)
            {
                ...
                f32 glide_amount = 1.0f-(moveVars.fallCount / f32(glide_down_time * 2));
    ...
    else if ( (pressed_a1 || swordState) && !moveVars.wallsliding ) //no attacking during a slide
    {
		if (getNet().isClient())
		{
			...
			{
				Sound::Play("AnimeMace.ogg", pos, myplayer ? 1.3f : 0.7f );
			}
			else if(knight.swordTimer == KnightVars::slash_charge)
			{
				Sound::Play("MaceSheath.ogg", pos, myplayer ? 1.3f : 0.7f );
			}
		}
				...
				knight.state <= KnightStates::sword_cut_down) // cut state
        {
            if (delta == DELTA_BEGIN_ATTACK) {
                Sound::Play( "/MaceSlash", this.getPosition() );
            }

            if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
            {
                f32 attackarc = 90.0f; // 90.0f
                ...
                DoAttack(this, 0.75f, attackAngle, attackarc, XORRandom(512) < 128 ? Hitters::mace_power : Hitters::mace, delta, knight);
            }
            else if (delta >= 9)
            {
                knight.swordTimer = 0;
				knight.state = KnightStates::sword_drawn;
                 ...
                 knight.state == KnightStates::sword_power_super)
        {
		    bool power_attack = knight.state == KnightStates::sword_power_super;
		    
            //attacking + noises
            if (delta == 2)
            {
                Sound::Play( "/ArgLong", this.getPosition() );
                Sound::Play( power_attack ? "/MacePowerSlash" : "/MaceSlash", this.getPosition() );
            }
            else if (delta > DELTA_BEGIN_ATTACK && delta < 10)
            {
			    //  Mace power attack
                if (power_attack)
				    DoAttack(this, 5.4f, -(vec.Angle()), 120.0f, Hitters::mace_power, delta, knight);
				else
				    DoAttack(this, 2.5f, -(vec.Angle()), 120.0f, XORRandom(512) < 128 ? Hitters::mace_power : Hitters::mace, delta, knight);
            }
            else if (delta >= KnightVars::slash_time || (knight.doubleslash && delta >= KnightVars::double_slash_time))
            {
                knight.swordTimer = 0;
                knight.state = KnightStates::sword_drawn;
            }
    ...
    if ( myplayer )
    {
		...
		{
			SetHelp( this, "help self action", this.getName(), "$Slash$ Slash!    $KEY_HOLD$$LMB$", "", 13 ); 
		}
    }
...
{
	if (hitterBlob.getName() == "magic_orb" && hitterBlob.getTeamNum() == this.getTeamNum())
		return 0.0f;

    // play cling sound if other knight attacked us
    // dmg could be taken out here if we ever want to

    if (hitterBlob.getPosition().x < this.getPosition().x && isClassTypeKnight(hitterBlob)) // knight and the left one (to play only once)
    {
                ...
                hsprite.isAnimation( "strike_down" ) || hsprite.isAnimation( "strike_up" ) )
        {
            if (sprite.isAnimation( "strike_power_ready" ) || sprite.isAnimation( "strike_mid" ) ||
                ...
                this.getSprite().PlaySound( "MaceCling" );
            }
        }
    }
	...
	return damage * modifier; //no block, damage goes through
}
	...
	if (map.getHitInfosFromArc( pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos ))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			{					
				...
				if (isTeamFriendly(this.getTeamNum(), b.getTeamNum()) && !b.hasTag("dead"))
				{ // no TK
					if(large)
						dontHitMore = true;
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1 )
{
	...
	if (blob is null || !solid || isTeamFriendly(this.getTeamNum(), blob.getTeamNum()))
	{
		return;
	}
...
bool hasItem( CBlob@ this, const string &in name )
{
	...
	if (inv !is null)
	{
		...
		warn("our inventory was null! 1g1d5la.as");
	}
...
void TakeItem( CBlob@ this, const string &in name )
{
	...
	if (inv !is null)
	{
		if (hasRequirements( inv, reqs, missing )) {
			...
			warn("took a bomb even though we dont have one! 1g1d5la.as");
		}
	}
	else {
		warn("our inventory was null! 1g1d5la.as");
...
void onAddToInventory( CBlob@ this, CBlob@ blob )
{
	...
	if (this.isMyPlayer() && this.getInventory().getItemsCount() > 1)
	{
		...
		{	  
			...
			{
				SetHelp( this, "help inventory", this.getName(), "$Help_Bomb1$$Swap$$Help_Bomb2$         $KEY_TAP$$KEY_F$", "", 2 );
				break;
			}
		}
	}
...
void KnightEncumbrance(CBlob@ this, RunnerMoveVars@ moveVars)
{
	moveVars.walkFactor *= getKnightSpeedModifier(this.getTeamNum());
	moveVars.jumpFactor *= getKnightJumpModifier(this.getTeamNum());
}
