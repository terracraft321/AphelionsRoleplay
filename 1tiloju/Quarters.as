...
void onInit(CBlob@ this)
{
	...
	{
		ShopItem@ s = addShopItem(this, "Beer", "$quarters_beer$", "beer", "A refreshing mug of beer.\nHeals 1 $heart$", false);
		AddRequirement(s.requirements, "coin", "", "Coins", cost_beer);
	}
	{
		ShopItem@ s = addShopItem(this, "Meal", "$quarters_meal$", "meal", "A hearty meal to get you back on your feet.\nHeals all $heart$", false);
		s.spawnNothing = true;
		s.customButton = true;
		s.buttonwidth = 2;
		s.buttonheight = 1;
	...
	{
		ShopItem@ s = addShopItem(this, "Egg", "$quarters_egg$", "egg", "A suspiciously undercooked egg, maybe it will hatch.\nHeals 3 $heart$", false);
		AddRequirement(s.requirements, "coin", "", "Coins", cost_egg);
	}
	{
		ShopItem@ s = addShopItem(this, "Burger", "$quarters_burger$", "burger", "A burger to go.\nHeals 3 $heart$", false);
...
void GetButtonsFor(CBlob@ this, CBlob@ caller)
{
	...
	if (!this.isOverlapping(caller) || !bedAvailable(this) || !requiresTreatment(caller)) {
		...
		caller.CreateGenericButton("$rest$", Vec2f(-12, 0), this, this.getCommandID("rest"), "Rest", params);
	}
