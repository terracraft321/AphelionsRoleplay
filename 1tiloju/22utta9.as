...
void onInit( CBlob@ this )
{
    ...
    vars.growth_time = 180 * 30;
    vars.height = 0;
    vars.max_height = 10 + XORRandom( 5 );
    vars.grown_times = 0;
    vars.max_grow_times = 133;
...
void GrowSprite( CSprite@ this, TreeVars@ vars )
{
    ...
    for (uint i = 0; i < segments.length; i++)
    {
        ...
        if (segment !is null && !segment.gotsprites)
        {
            ...
            {
                CSpriteLayer@ newsegment = this.addSpriteLayer( "segment "+i, getSpritePath(), 32, 16, 0, 0 );
            ...
            else if (segment.grown_times == 3 && i > 0 && i != vars.max_height-1)
            {
                ...
                {
                    CSpriteLayer@ newsegment = this.addSpriteLayer( "leaves "+i+" "+spriteindex, getSpritePath(), 32, 32, 0, 0 );
			...
			{
				f32 flipsign = 1.0f;
				CSpriteLayer@ newsegment = this.addSpriteLayer( "roots", getSpritePath(), 32, 16, 0, 0 );
            ...
            {
                CSpriteLayer@ newsegment = this.addSpriteLayer( "extra leaves top", getSpritePath(), 32, 32, 0, 0 );
            ...
            else if (segment.grown_times == 5 && i == vars.max_height-1) //top of the tree
            {
                ...
                {
                    CSpriteLayer@ newsegment = this.addSpriteLayer( "leaves "+i+" "+3, getSpritePath(), 64, 32, 0, 0 );
                ...
                {
                    CSpriteLayer@ newsegment = this.addSpriteLayer( "leaves "+i+" "+4, getSpritePath(), 64, 32, 0, 0 );
