...
void ReloadSprite( CSprite@ this )
{
    ...
    string texname = "Entities/Industry/" + RP_NAME + "/Chainsaw/Chainsaw.png";
	if(isTechResearched("Machinery III", blob.getTeamNum()))
	    texname = "Entities/Industry/" + RP_NAME + "/Chainsaw/ChainsawAdamant.png";
	else if(isTechResearched("Machinery II", blob.getTeamNum()))
	    texname = "Entities/Industry/" + RP_NAME + "/Chainsaw/ChainsawMythril.png";
	else if(isTechResearched("Machinery I", blob.getTeamNum()))
	    texname = "Entities/Industry/" + RP_NAME + "/Chainsaw/ChainsawSteel.png";
					...
					if (map.getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 30, distance, this, false, @hitInfos))
					{
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (hi.blob !is null) // blob
							{
								...
								hitsomething = true;
								hit_wood = hi.blob.getName() == "tree_bushy" || hi.blob.getName() == "tree_pine" || hi.blob.getName() == "tree_redwood" || hi.blob.getName() == "log";
							}
							else // map
							{
